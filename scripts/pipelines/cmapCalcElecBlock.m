function cmapCalcElecBlock(pname, struct_file, pH, srcutoff, BlockSize)
    ...
    %% Input Parameters
    pdblist = char(pname); % Input PDB filename
    stridefile = char(struct_file); % File containing raw output from the STRIDE server
    % pH = 7; % pH value
    ...
    cdata=fullfile(pwd, '..', 'data/', cdata);
    if ~isfile(cdata)
        error("ERROR : File %s not found.", cdata)
    end
    pdbr=fopen(cdata,'rt'); % Raw PDB data
    buffer = fullfile(pwd, '..', 'data', 'WSME_outputs', 'buffer1.txt');
    pdbc=fopen(buffer,'wt'); % Curated PDB data
    ...
    %% Printing BlockDet
    BlockDetPath = fullfile(pwd, '..', 'data', 'WSME_outputs', [pdblist '_BlockDet.dat']);
    eval(['save ',char(BlockDetPath),' BlockMat -ascii']);
    %% Printing Regular Comtact Map files
    opPath = fullfile(pwd, '..', 'data', 'WSME_outputs', ['contactmapmatElec',deblank(pdblist(1,:)),'.dat']);
    eval(['save ',char(opPath),' srcont -ascii']);
    opPath = fullfile(pwd, '..', 'data', 'WSME_outputs', ['contdistElec',deblank(pdblist(1,:)),'.dat']);
    eval(['save ',char(opPath),' ElecMat -ascii']);
    opPath = fullfile(pwd, '..', 'data', 'WSME_outputs', ['contdistElecB',deblank(pdblist(1,:)),'.dat']);
    eval(['save ',char(opPath),' contdistMapB -ascii']);
    opPath = fullfile(pwd, '..', 'data', 'WSME_outputs', ['contactmapmatElecB',deblank(pdblist(1,:)),'.dat']);
    eval(['save ',char(opPath),' contactMapB -ascii']);
    opPath = fullfile(pwd, '..', 'data', 'WSME_outputs', ['BlockSize',deblank(pdblist(1,:)),'.dat']);
    eval(['save ',char(opPath),' BlockSize -ascii']);   
